...
public class ReportServiceImpl extends AbstractCrudService<ReportRepository, Report, ReportExample, Integer> implements ReportService {
	@Resource
	...
	@Override
	public Report getByUcode(final String ucode) {
		if (StringUtils.isBlank(ucode)) {
			return null;
		}
		//
		final ReportExample example = new ReportExample();
		example.or().andUcodeEqualTo(ucode);
		return this.dao.selectOneByExample(example);
	}
	...
	public ReportDataSource getReportDataSource(final int dsId) {
		...
		return new ReportDataSource(ds.getUid(), ds.getDriverClass(), ds.getJdbcUrl(), ds.getUser(), ds.getPassword(), ds.getQueryerClass(), ds.getPoolClass(), options);
	}
}
