...
public class BootstrapQueryFormView extends AbstractQueryParamFormView implements QueryParamFormView {
	...
	protected String getDateBoxText(final HtmlDateBox dateBox) {
		...
		htmlText.append(String.format("<input id=\"%s\" name=\"%s\" class=\"hasDatepicker\" type=\"text\" value=\"%s\" readonly />", dateBox.getName(), dateBox.getName(), dateBox.getValue()));
	...
	protected String getTexBoxText(final HtmlTextBox textBox) {
		...
		htmlText.append("<label class=\"input\">");
		htmlText.append(String.format("<input id=\"%s\" name=\"%s\" type=\"text\" value=\"%s\" placeholder=\"%s\"/>", textBox.getName(), textBox.getName(), textBox.getValue(), textBox.getDefaultText()));
		htmlText.append("</label>");
		htmlText.append("</div>");
		return htmlText.toString();
	}
	...
	protected String getCheckBoxText(final HtmlCheckBox checkBox) {
		...
		htmlText.append(String.format("<input id=\"%s\" name=\"%s\" class=\"checkbox-item\" type=\"checkbox\" value=\"%s\" %s />", checkBox.getName(), checkBox.getName(), checkBox.getValue(), checked));
	...
	protected String getCheckboxListText(final HtmlCheckBoxList checkBoxList) {
		...
		for (final HtmlCheckBox checkBox : checkBoxList.getValue()) {
			...
			final String checked = checkBox.isChecked() ? "checked=\"checked\"" : "";
			htmlText.append(String.format("<label class=\"checkbox\">%s", checkBox.getText()));
			htmlText.append(String.format("<input class=\"checkbox-item\" type=\"checkbox\" name=\"%s\" value=\"%s\" %s />", checkBoxList.getName(), checkBox.getName(), checked));
			htmlText.append("<i></i></label>");
		}
		htmlText.append("<label class=\"checkbox\">全选 ");
		htmlText.append(String.format("<input type=\"checkbox\" class=\"checkAllStatColumn\" name=\"checkAllStatColumn\" " + "id=\"checkAllStatColumn\" %s /><i></i></label>", isCheckedAll ? "checked=\"checked\"" : ""));
		return htmlText.toString();
	}
	...
	private String getAutoCompleteComboBoxText(final HtmlComboBox comboBox) {
		...
		htmlText.append(String.format("<label>%s:</label>", comboBox.getText()));
		htmlText.append("<label class=\"input\">");
		htmlText.append(String.format("<input type=\"text\" list=\"%1$s\" name=\"%1$s\" value=\"%2$s\">", comboBox.getName(), comboBox.getDefaultValue()));
