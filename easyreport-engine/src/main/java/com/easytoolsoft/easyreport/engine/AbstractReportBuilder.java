...
public abstract class AbstractReportBuilder implements ReportBuilder {
		...
		for (int rowIndex = 0; rowIndex < rowCount; rowIndex++) {
			...
			if (rowIndex == 0) {
				for (final ReportDataColumn leftColumn : leftFixedColumns) {
					this.tableRows.append(String.format("<th title=\"%s\"%s>%s</th>", leftColumn.getMetaData().getComment(), rowSpan, leftColumn.getText()));
				}
			}
			for (final ColumnTreeNode rightColumn : rightColumnTree.getNodesByLevel(rowIndex)) {
				final String colSpan = rightColumn.getSpans() > 1 ? String.format(" colspan=\"%s\"", rightColumn.getSpans()) : "";
				this.tableRows.append(String.format("<th title=\"%s\"%s style=\"%s\">%s</th>", rightColumn.getColumn().getMetaData().getComment(), colSpan, rightColumn.getStyle(), rightColumn.getValue()));
			}
	 ...
	 * @return
	 */
	protected String[] drawLeftFixedColumn(final Map<String, ColumnTreeNode> treeNodePathMap, final String[] lastNodePaths, final ColumnTreeNode rowNode, final boolean isRowSpan) {
		if (isRowSpan) {
			return this.drawLeftRowSpanColumn(treeNodePathMap, lastNodePaths, rowNode);
		}

		final String[] paths = StringUtils.splitPreserveAllTokens(rowNode.getPath(), this.reportDataSet.getPathSeparator());
	 ...
	 * @return 当前跨行结点的树路径
	 */
	protected String[] drawLeftRowSpanColumn(final Map<String, ColumnTreeNode> treeNodePathMap, String[] lastNodePaths, final ColumnTreeNode rowNode) {
		final String[] paths = StringUtils.splitPreserveAllTokens(rowNode.getPath(), this.reportDataSet.getPathSeparator());
		if (paths == null || paths.length == 0) {
			return null;
		}
