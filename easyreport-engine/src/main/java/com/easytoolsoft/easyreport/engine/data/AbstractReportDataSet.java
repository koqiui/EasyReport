...
public abstract class AbstractReportDataSet implements ReportDataSet {
	...
	public ColumnTree getStatColumnTree() {
		...
		treeNodes.addAll(statColumns.stream().filter(statColumn -> !statColumn.getMetaData().isHidden()).map(this::createStatColumnTreeNode).collect(Collectors.toList()));
	...
	public List<ReportDataColumn> getEnabledStatColumns() {
		...
		this.enabledStatColumns.addAll(this.getStatColumns().stream().filter(statColumn -> !statColumn.getMetaData().isHidden()).collect(Collectors.toList()));
		return this.enabledStatColumns;
	}
	...
	public List<ReportDataColumn> getComputedColumns() {
		...
		this.computedColumns.addAll(this.getStatColumns().stream().filter(statColumn -> statColumn.getMetaData().getType() == ColumnType.COMPUTED).collect(Collectors.toList()));
		return this.computedColumns;
	}
	...
	public Map<String, ReportDataRow> getRowMap() {
		...
		for (ReportMetaDataRow metaDataRow : metaDataRows) {
			...
			for (ReportDataColumn column : computedColumns) {
				...
				// 收集计算列的值
				column.getMetaData().clctCellValue(value);
		...
		// 初始化计算列格式信息
		for (ReportDataColumn column : computedColumns) {
			column.getMetaData().initFormatInfo();
		}
		//
		return dataRowMap;
	}
	...
	protected ColumnTreeNode createColumnTreeNode(ReportMetaDataRow metaDataRow, List<ReportDataColumn> columns, int depth, String path) {
	...
	protected int setSpansAndDepthByRecursion(ColumnTreeNode parentNode, int depth, boolean isSort, List<ReportDataColumn> columns) {
	...
	protected void sortTreeNodesByLevel(List<ColumnTreeNode> treeNodes, int level, List<ReportDataColumn> columns) {
		...
		final ColumnSortType sortType = column.getMetaData().getSortType();
		if (sortType == ColumnSortType.DEFAULT || columnType == ColumnType.STATISTICAL || columnType == ColumnType.COMPUTED) {
			return;
		}
